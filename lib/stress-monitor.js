...
class StressMonitor {
  constructor(config = {}) {
    this.config = {
      ...
      recovery: {
        ...
        recoveryCheckInterval: config.recovery?.recoveryCheckInterval || 30000,
      }
    };
  ...
  recordOperation(type, duration, success = true) {
    ...
    if (isCritical) {
      ...
      // Successful fast operation - decay stress indicators
      this.consecutiveSlowOps = Math.max(0, this.consecutiveSlowOps - 1);
      this.criticalEvents = Math.max(0, this.criticalEvents - 1);
    }
