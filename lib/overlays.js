...
/**
 * SVG Template Cache
 * Caches SVG templates by style configuration to avoid regenerating
 * identical SVG structures every frame.
 */
...
function generateTextSVG(text, overlay, region) {
  ...
  if (!template) {
    // Build template with placeholder for text
    template = `
  ...
  // Replace text placeholder and return buffer
  return Buffer.from(template.replace('{{TEXT}}', text));
}
