...
class DisplayScheduler {
  ...
  async displayFrame(displaySecond) {
    ...
    if (!operation) {
      ...
      console.warn(`⚠️  DROPPED FRAME: No operation queued for second ${displaySecond} (${new Date(displaySecond * 1000).toISOString()})`);
      const status = this.queue.getStatus(displaySecond);
      console.warn(`    Queue status: ${status.size} operations, range: ${status.range}`);
