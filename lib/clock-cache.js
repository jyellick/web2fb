...
class ClockCache {
  ...
  async preRender(startTime = new Date(), count = this.windowSize) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Get base region dimensions to ensure overlay matches exactly
      const baseMetadata = await sharp(this.baseRegionBuffer).metadata();

      // Ensure overlay matches base region dimensions exactly
      // Sharp's SVG rendering can produce slightly different sizes
