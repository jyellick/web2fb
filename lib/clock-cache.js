...
class ClockCache {
  constructor(overlay, baseRegionBuffer, region, detectedStyle = {}) {
    ...
    this.baseRegionMetadata = null; // Cache base region dimensions
  ...
  async preRender(startTime = new Date(), count = this.windowSize) {
    ...
    // Cache base region dimensions once (avoids repeated metadata calls)
    ...
    for (let i = 0; i < count; i++) {
      ...
      const compositePipeline = sharp(this.baseRegionBuffer)
        .composite([{ input: finalOverlay }]);

      // Get actual metadata from composited image (channels might differ from base)
      const compositeMetadata = await compositePipeline.metadata();

      // Now convert to raw with correct metadata
      const compositeImage = await compositePipeline
        .raw()
        .toBuffer();

      // Store raw buffer with actual metadata from the composited result
      this.frames.set(frameSecond, {
        buffer: compositeImage,
        width: compositeMetadata.width,
        height: compositeMetadata.height,
        channels: compositeMetadata.channels
      });
    }
