...
class ClockCache {
   ...
   */
  updateBaseRegion(newBaseRegionBuffer, rawMetadata = null) {
  ...
  /**
   * Swap base region buffer without invalidating existing cache
   * Used when transitioning to new base image - keeps existing frames,
   * new frames will be rendered with new base as cache extends naturally
   */
  swapBaseRegion(newBaseRegionBuffer, rawMetadata = null) {
    this.baseRegionBuffer = newBaseRegionBuffer;
    this.rawMetadata = rawMetadata;
    this.needsResize = null; // Reset resize detection for new base
    // DON'T invalidate - keep existing frames, future extensions use new base
  }
}
