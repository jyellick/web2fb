...
class ClockCache {
  ...
  async preRender(startTime = new Date(), count = this.windowSize) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Store raw buffer with metadata needed for Sharp reconstruction
      this.frames.set(frameSecond, {
        buffer: compositeImage,
        width: this.baseRegionMetadata.width,
        height: this.baseRegionMetadata.height,
        channels: this.baseRegionMetadata.channels
      });
    }
