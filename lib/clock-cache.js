...
class ClockCache {
  ...
  async preRender(startTime = new Date(), count = this.windowSize) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      // First composite to PNG to let Sharp determine final format
      const compositePng = await sharp(this.baseRegionBuffer)
        .composite([{ input: finalOverlay }])
        .png()
        .toBuffer();

      // Get metadata from the actual composited result (not pipeline input)
      const compositeMetadata = await sharp(compositePng).metadata();

      // Convert to raw with correct metadata
      const compositeImage = await sharp(compositePng)
        .raw()
        .toBuffer();
