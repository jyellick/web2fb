...
class OverlayManager {
   ...
   *
   * @param {boolean} allowCacheExtension - Whether to allow cache to extend (default: true)
   */
  createUpdateFunction(overlay, framebuffer, onBaseTransition, allowCacheExtension = true) {
    return async () => {
      const state = this.states.get(overlay.name);
      if (!state || !state.baseRegionBuffer) {
        console.error(`No state or baseRegionBuffer for overlay '${overlay.name}'`);
        return;
      }
      ...
      if (overlay.type === 'clock') {
        const cache = this.clockCaches.get(overlay.name);
        if (cache) {
          if (allowCacheExtension && cache.needsMoreFrames()) {
            ...
            try {
              ...
              this.perfMonitor.end(extendOpId, { success: false });
              console.error(`Error extending cache window for '${overlay.name}':`, err.message);
              throw err;
            }
          }
  ...
  /**
   * Swap base regions in overlay states and clock caches without invalidating caches
   * This allows existing cached frames to continue being used while new frames
   * are rendered with the new base as the cache naturally extends
   */
  swapBaseRegions(newStates) {
    // Update overlay states with new base region buffers
    for (const [name, newState] of newStates.entries()) {
      this.states.set(name, newState);

      // For clock overlays, swap the base region in the cache without invalidating
      const cache = this.clockCaches.get(name);
      if (cache) {
        cache.swapBaseRegion(newState.baseRegionBuffer, newState.rawMetadata);
      }
    }
  }
}
