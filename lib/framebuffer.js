...
class Framebuffer {
  ...
  async writePartial(imageBuffer, region) {
    ...
    try {
      ...
      // Batch writes for better syscall efficiency
      // Write multiple lines at a time to reduce syscall overhead
      const linesPerBatch = 10;

      for (let y = 0; y < regionHeight; y += linesPerBatch) {
        const linesToWrite = Math.min(linesPerBatch, regionHeight - y);
        const srcOffset = y * bytesPerLine;
        const fbOffset = ((region.y + y) * fbBytesPerLine) + (region.x * this.info.bytesPerPixel);
        const bytesToWrite = linesToWrite * bytesPerLine;

        fs.writeSync(this.fd, rawBuffer, srcOffset, bytesToWrite, fbOffset);
      }
