...
class LocalScreenshotProvider extends ScreenshotProvider {
  ...
  async captureScreenshot(hideSelectors = []) {
    ...
    try {
      ...
      let waitUntil = browserConfig.waitForNetworkIdle ? 'networkidle0' : 'networkidle2';
      ...
      try {
        ...
        // If networkidle0 failed with frame detached, retry with networkidle2
        if (waitUntil === 'networkidle0' && err.message.includes('frame was detached')) {
          console.log('Local browser: Retrying navigation with networkidle2 (more forgiving)...');
          ...
          console.log('Local browser: Navigation complete (fallback to networkidle2)');
        } else {
          throw err;
        }
      }
      ...
      // Use manual polling instead of waitForFunction to avoid page JavaScript
      // interference with Puppeteer's internal polling variables
      ...
      while (!stable && attempts < maxAttempts) {
        ...
        try {
          ...
          console.log(`Local browser: Stability check failed (attempt ${attempts + 1}): ${err.message}`);
        }
        attempts++;
      }

      if (!stable) {
        throw new Error('Local browser: Page failed to stabilize after 30 seconds');
      }
      console.log(`Local browser: Page stabilized after ${attempts * 500}ms`);

      // Wait for specific selector(s) if provided
      // Use manual polling to avoid page JavaScript interference with Puppeteer's internal variables
      if (browserConfig.waitForSelector) {
        ...
        const maxSelectorAttempts = 240; // 120 seconds max (240 * 500ms)
        ...
        while (!selectorsFound && selectorAttempts < maxSelectorAttempts) {
          ...
          try {
            ...
            // Page environment not ready yet, continue polling
            console.log(`Local browser: Selector check failed (attempt ${selectorAttempts + 1}): ${err.message}`);
          }
          selectorAttempts++;
        }

        if (!selectorsFound) {
          throw new Error(`Local browser: Selectors not found after 120 seconds: ${selectors.join(', ')}`);
        }
        console.log(`Local browser: All selectors found after ${selectorAttempts * 500}ms`);
      }

      // Wait for images using manual polling
      console.log('Local browser: Waiting for images to load...');
      let imagesLoaded = false;
      let imageAttempts = 0;
      const maxImageAttempts = 240; // 120 seconds max (240 * 500ms)
      ...
      if (!imagesLoaded) {
        throw new Error('Local browser: Images failed to load after 120 seconds');
      }
