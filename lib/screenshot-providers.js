...
class LocalScreenshotProvider extends ScreenshotProvider {
  ...
  async initialize() {
    ...
    await this.page.goto(this.config.display.url, {
      waitUntil: 'load',
      timeout: browserConfig.timeout || 180000
    });

    // Scroll to trigger lazy loading
    const performanceConfig = this.config.performance || {};
    if (performanceConfig.scrollToLoadLazy !== false) {
    ...
    // Wait for images
    if (performanceConfig.waitForImages !== false) {
      console.log('Waiting for images to load...');
      await this.page.waitForFunction(() => {
        const images = Array.from(document.images);
        return images.every(img => img.complete && img.naturalHeight !== 0);
      }, { timeout: browserConfig.imageLoadTimeout || 120000 });
      console.log('All images loaded');
    }

    // Disable animations
    if (browserConfig.disableAnimations !== false) {
  ...
  async setupChangeDetection(onChangeCallback) {
    ...
    await this.page.evaluate((config) => {
      ...
      function captureSnapshot() {
        const elements = document.querySelectorAll(config.watchSelectors.join(','));
      ...
      console.log('Change detection initialized with debouncing:', config.debounceDelay || 500, 'ms');
      ...
      const observer = new MutationObserver((mutations) => {
        ...
        for (const mutation of mutations) {
          if (mutation.type === 'attributes') {
            ...
            if (config.watchAttributes.includes(attrName)) {
              shouldCheck = true;
              break;
            }
          }
      ...
      observer.observe(document.body, {
        attributes: true,
        attributeFilter: config.watchAttributes,
        childList: true,
        subtree: true
      });
      ...
      console.log(`Change detection active (periodic check: ${config.periodicCheckInterval}ms, debounce: ${config.debounceDelay || 500}ms)`);
...
class RemoteScreenshotProvider extends ScreenshotProvider {
  ...
  async captureScreenshot(hideSelectors = []) {
    ...
    const params = new URLSearchParams({
      ...
      waitForImages: (performanceConfig.waitForImages !== false).toString()
    });
