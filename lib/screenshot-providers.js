...
class LocalScreenshotProvider extends ScreenshotProvider {
  ...
  async captureScreenshot(hideSelectors = []) {
    ...
    try {
      ...
      // Simple wait after load instead of scrolling (which causes frame detachment with DakBoard)
      // DakBoard's heavy scroll handlers and lazy loading cause the frame to become detached
      console.log('Local browser: Waiting for page to render after load...');
      await new Promise(resolve => setTimeout(resolve, 5000)); // Wait 5 seconds for initial render
      console.log('Local browser: Initial render wait complete');

      // Wait for specific selector(s) if provided (best effort - don't fail if frame detaches)
      ...
      try {
        console.log('Local browser: Disabling animations...');
      ...
      // Hide overlay elements (best effort)
