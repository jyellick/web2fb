...
class LocalScreenshotProvider extends ScreenshotProvider {
  ...
  async captureScreenshot(hideSelectors = []) {
    ...
    try {
      ...
      // DakBoard closes the session/frame within seconds of load
      // Skip all optional waits and take screenshot immediately
      console.log('Local browser: Taking screenshot immediately (DakBoard closes session quickly)...');

      // Skip selector/image waiting - just screenshot NOW
      /*
      // Disabled: DakBoard closes frame too quickly for these checks
      if (browserConfig.waitForSelector) {
        ...
        const maxSelectorAttempts = 60; // 30 seconds max (60 * 500ms)
        ...
        while (!selectorsFound && selectorAttempts < maxSelectorAttempts) {
          ...
          try {
            ...
            // Frame detached or other error - give up and continue
            if (err.message.includes('detached')) {
              console.log(`Local browser: Frame detached during selector wait, continuing anyway`);
              break;
            }
          }
          selectorAttempts++;
        ...
        if (selectorsFound) {
          console.log(`Local browser: All selectors found after ${selectorAttempts * 500}ms`);
        } else {
          console.log(`Local browser: Selector wait timed out or failed, continuing anyway`);
        }
      }

      // Wait for images using manual polling (best effort - don't fail if frame detaches)
      console.log('Local browser: Waiting for images to load...');
      let imagesLoaded = false;
      let imageAttempts = 0;
      const maxImageAttempts = 60; // 30 seconds max (60 * 500ms)

      while (!imagesLoaded && imageAttempts < maxImageAttempts) {
        await new Promise(resolve => setTimeout(resolve, 500));
        try {
          ...
          // Frame detached - DakBoard does this, just continue
          if (err.message.includes('detached') || err.message.includes('Target closed')) {
            console.log(`Local browser: Frame detached during image wait, continuing anyway`);
            break;
          }
          console.log(`Local browser: Image check failed (attempt ${imageAttempts + 1}): ${err.message}`);
        }
        imageAttempts++;
      }

      if (imagesLoaded) {
        console.log(`Local browser: All images loaded after ${imageAttempts * 500}ms`);
      } else {
        console.log(`Local browser: Image wait timed out or failed, continuing anyway`);
      }
      */

      // Skip all delays and style modifications - DakBoard closes session too fast
      // Just take the screenshot immediately after navigation
      ...
      console.log(`Local browser: Screenshot captured (${screenshot.length} bytes)`);
      return screenshot;
      ...
      if (userDataDir) {
        try {
          ...
          console.log(`Local browser: Cleaned up temp profile: ${userDataDir}`);
        } catch (err) {
          console.log(`Local browser: Profile cleanup error: ${err.message}`);
        }
      }
